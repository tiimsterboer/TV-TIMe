...
class CosSim {
    ...
    func wordCount (r: String, s: String) -> (wordBag: Array<String>, freq1: Dictionary<String, Double>, freq2: Dictionary<String, Double>) {
        let words1 = cleanText(txt: r).components(separatedBy: " ")
        let words2 = cleanText(txt: s).components(separatedBy: " ")
        let wordBag = Array(Set(words1 + words2))
        var wordDct1 = Dictionary<String, Double>()
        var wordDct2 = Dictionary<String, Double>()
        ...
        for word in wordBag {
            ...
            for x in words1 {
                if x == word {
                    wordDct1.updateValue(wordDct1[word]!, forKey: word)
                }
            }
            for x in words2 {
                if x == word {
                    wordDct2.updateValue(wordDct2[word]!, forKey: word)
                }
            }
        }
        let freq1 = wordDct1
    ...
    func cosSim (wordBag: Array<String>, freq1: Dictionary<String, Double>, freq2: Dictionary<String, Double>) -> Double {
        var top: Double = 0
        var bot1: Double = 0
        var bot2: Double = 0
